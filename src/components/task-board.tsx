  ...
  return (
    <div className="space-y-4">
      ...
      {!isLoading && data && (
        <div className="kanban-columns">
          {columns.map((col) => (
            <div key={col.status} className="bg-ground-100/50 rounded-xl p-3 flex flex-col min-h-[200px]">
              {/* Column header */}
              ...
              <div className="flex-1 space-y-2 overflow-y-auto">
                ...
                {col.tasks.length === 0 && (
                  <div className="flex items-center justify-center py-8">
                    <p className="text-xs text-neutral-400 italic">No tasks</p>
                  </div>
                )}
              </div>
            </div>
    ...
    <div className="rounded-xl bg-white shadow-card py-20 px-6 text-center">
      ...
      <p className="text-xs text-neutral-400 max-w-xs mx-auto">
        {hasFilters
          ...
          : "Submit tasks on the home page to have them analyzed, categorized, and prioritized by AI."}
      </p>
      {!hasFilters && (
